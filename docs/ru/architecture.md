...
| `412 Precondition Failed` | `409 context_stale` — **и свежий контекст в том же ответе** |
| запись без `If-Match` проходит | запись без версии отклоняется: `428 context_version_required` |

Два отличия от классического ETag сделаны сознательно. Во-первых, `409` возвращает контекст сразу — агенту не нужен второй round-trip, чтобы перечитать состояние. Во-вторых, отсутствие версии — это **ошибка**, а не разрешение писать: забытое поле не должно двигать реальные цены.
...
- **Карантин при резком падении цены.** WB кладёт товар в ценовой карантин при снижении примерно в 1,5–3 раза, и новая цена просто не применяется. В `repricer.cjs` зашито ступенчатое снижение: `QUAR_RATIO = 1.5`, и если цель ниже текущей более чем в 1,5 раза, за прогон цена опускается только до `⌈current / 1.45⌉` (`QUAR_STEP = QUAR_RATIO − 0.05`, чтобы не сесть ровно на границу). Остаток добирается следующими прогонами. Параллельно `fetchQuarantine()` читает `/api/v2/quarantine/goods` и пишет предупреждение в лог синка.
