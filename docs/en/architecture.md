...
- **Quarantine on a sharp price drop.** WB quarantines an item when the price falls by roughly 1.5×–3×, and the new price simply is not applied. The step limit lives in `lib/priceStep.cjs`, shared by the repricer and the strategy engine: when the target is below the threshold, a run lowers the price only to `⌈current / 1.45⌉` (a 0.05 margin under the 1.5 threshold, so it never lands exactly on it), and later runs finish the job. The threshold is per store (`max_drop_ratio`), and only Wildberries has a default — Ozon and Yandex have no measured threshold, and inventing one is worse than leaving the guard off. The opposite limit, `max_raise_percent` (20% per run), keeps a price from leaping upwards; the previous setting name `ym_floor_max_raise_percent` is still honoured. In parallel, `fetchQuarantine()` reads `/api/v2/quarantine/goods` and writes a warning into the sync log.
