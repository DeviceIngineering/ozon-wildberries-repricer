...
**Резкое изменение цены — ступенчато.** Wildberries отправляет товар в карантин, если цена падает более чем в полтора раза: цена просто не применится, а товар выпадет из выдачи. Репрайсер знает про этот порог и, если цель ниже, идёт к ней за несколько прогонов, каждый раз оставаясь чуть мягче порога. То же ограничение действует и в ценовых экспериментах — там это отдельно помечается в логе как `карантин-кап`.
